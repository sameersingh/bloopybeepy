...
abstract class MessageFactor(val factor: Factor, val varying: Set[DiscreteVariable], val fg: LatticeBP) {
  ...
  protected def getScore(assignment: Assignment, index: Int = -1): Double = {
    if (index >= 0 && fg.usesScoreCaching) {
      if (_cache(index).isNaN) _cache(index) = factor.assignmentScore(assignment)
      _cache(index)
    } else factor.assignmentScore(assignment)
  }
  ...
  def foldMarginals[A](init: A)(f: (A, Assignment, Double) => A): A = {
    var a: A = init
    for (i <- 0 until values.length) {
      val index = indices(i)
      val assignment = values(i)
  ...
  def maxAsMarginal = {
    variable match {
      case dv: DiscreteVariable => {
        marginalize
        _marginal = BPUtil.deterministicMessage(dv, _marginal.map[dv.Value])
...
abstract class LatticeBP(val varying: Set[DiscreteVariable]) /*extends Lattice[Variable]*/ {
  ...
  def createUnrolled(model: Model) {
    createFactors(model.factors(varying).toSeq)
  }
  ...
  def setToMaxMarginal(variables: Iterable[MutableVar[DiscreteValue]] = varying.toSet)(implicit d: DiffList = null): Unit = {
    for (variable <- variables) {
      variable.set(node(variable).marginal.map[variable.Value])(d)
    }
  }

  def setToMaxMarginalWithThreshold[Var <: DiscreteVariable, Val <: Var#Value](variables: Iterable[Var],
                                                                                   threshold: Double,
                                                                                   default: Val)
